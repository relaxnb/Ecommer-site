                        ...
                        <div class="row">
                            <div class="col col-lg-6">
                                <ul class="btns_group ul_li_right">
                                    <li><button type="submit" class="btn border_black" href="#!">Update Cart</button></li>
                                </form>
                                    @php
                                        session([
                                            'discount'=>$discount,
                            ...
                            <div class="col col-lg-6">
                                <form action="{{ url('/cart') }}" method="GET">
                                    <div class="coupon_form form_item mb-0">
                                        <input type="text" name="coupon_code" value="{{ $coupon_code }}" placeholder="Coupon Code...">
...
@section('footer_script')
    ...
    <script>
        let quan_input = document.querySelectorAll('.abc')
        let arr = Array.from(quan_input)
        ...
        arr.map(item=>{
            item.addEventListener('click', function(e){
                if(e.target.className == 'fal fa-plus'){
                    ...
                    let quan = e.target.parentElement.previousElementSibling.value
                    let price = e.target.dataset.price
                    item.nextElementSibling.innerHTML = price*quan
                }
                if(e.target.className == 'fal fa-minus'){
                    if(e.target.parentElement.nextElementSibling.value > 1){
                        ...
                        let quan = e.target.parentElement.nextElementSibling.value
                        let price = e.target.dataset.price
                        item.nextElementSibling.innerHTML = price*quan
                    }
